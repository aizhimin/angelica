...
/**
 * 唯一ID生成机制，mongodb object id的实现机制。
 * @author aizhimin
 */
public class ObjectIdEntity implements Comparable<ObjectIdEntity>, Serializable {
	private static final long serialVersionUID = 3670079982654483072L;

	static final Logger LOGGER = Logger.getLogger("ObjectIdEntity");
	 ...
	 * @return the new id
	 */
	public static ObjectIdEntity get() {
	...
	/**
	 * <p>
	 * Creates an ObjectId using time, machine and inc values. The Java driver used
	 * to create all ObjectIds this way, but it does not match the
	 * <a href="http://docs.mongodb.org/manual/reference/object-id/">ObjectId
	 * specification</a>, which requires four values, not three. This major release
	 * of the Java driver conforms to the specification, but still supports clients
	 * that are relying on the behavior of the previous major release by providing
	 * this explicit factory method that takes three parameters instead of four.
	 * </p>
	 *
	 * <p>
	 * Ordinary users of the driver will not need this method. It's only for those
	 * that have written there own BSON decoders.
	 * </p>
	 *
	 * <p>
	 ...
	 * @since 2.12.0
	 */
	public static ObjectIdEntity createFromLegacyFormat(final int time, final int machine, final int inc) {
		return new ObjectIdEntity(time, machine, inc);
	}
	 ...
	 *            the date
	 */
	public ObjectIdEntity(final Date date) {
	 ...
	 */
	public ObjectIdEntity(final Date date, final int counter) {
	 ...
	 *             zero
	 */
	public ObjectIdEntity(final Date date, final int machineIdentifier, final short processIdentifier,
			final int counter) {
	 ...
	 *             zero
	 */
	public ObjectIdEntity(final int timestamp, final int machineIdentifier, final short processIdentifier,
			final int counter) {
		this(timestamp, machineIdentifier, processIdentifier, counter, true);
	}

	private ObjectIdEntity(final int timestamp, final int machineIdentifier, final short processIdentifier,
	 ...
	 *             ObjectId
	 */
	public ObjectIdEntity(final String hexString) {
	 ...
	 */
	public ObjectIdEntity(final byte[] bytes) {
//		this(ByteBuffer.wrap(notNull("bytes", bytes)));
	 ...
	 *            incremental value
	 */
	ObjectIdEntity(final int timestamp, final int machineAndProcessIdentifier, final int counter) {
	 ...
	 * @since 3.4
	 */
	public ObjectIdEntity(final ByteBuffer buffer) {
...
//		isTrueArgument("buffer.remaining() >=12", buffer.remaining() >= 12);
	...
	public boolean equals(final Object o) {
		...
		ObjectIdEntity objectId = (ObjectIdEntity) o;
	...
	@Override
	public int compareTo(final ObjectIdEntity other) {
		if (other == null) {
...
//		ObjectIdEntity objectId = ObjectIdEntity.get();
...
//		System.out.println(objectId.getDate().getTime());
